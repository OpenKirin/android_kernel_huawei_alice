...
static int k3_v4l2_open(struct file *file)
{
	...
	if (0 == cam->open_count++) {
		...
		/* init isp controller */
		ret = k3_isp_init(v4l2_ctl.pdev, &v4l2_ctl.data_queue, v4l2_ctl.module_pdev);
		if (ret != 0) {
...
/*
 **************************************************************************
 * FunctionName: k3_sensor_module_probe;
 * Description : probe isp driver;
 * Input       : NA;
 * Output      : NA;
 * ReturnValue : NA;
 ...
 **************************************************************************
 */
static /*__init*/ int k3_sensor_module_probe(struct platform_device *pdev)
{
	v4l2_ctl.module_pdev = pdev;
	print_info("K3 camera k3_sensor_module_probe init OK !!");
	return 0;
...
/*
 **************************************************************************
 * FunctionName: k3_sensor_module_remove;
 * Description : probe sensor module probe  driver;
 * Input       : NA;
 * Output      : NA;
 * ReturnValue : NA;
 * Other       : NA;
 **************************************************************************
 */
static /*__exit*/ int k3_sensor_module_remove(struct platform_device *pdev)
{
	print_info("enter K3 k3_sensor_module_remove ");

	v4l2_ctl.module_pdev = NULL;

	return 0;
}

static const struct of_device_id hisi_sensor_module_dt_match[] = {
	{.compatible = "hisi,hisi_sensor", },
	{}
};
MODULE_DEVICE_TABLE(of, hisi_sensor_module_dt_match);

static struct platform_driver k3_sensor_module_probe_driver = {
	.probe = k3_sensor_module_probe,
	.remove = /*__devexit_p*/(k3_sensor_module_remove),
	.shutdown = NULL,
	.driver = {
			.name = "hisi_sensor",
			.owner = THIS_MODULE,
			.of_match_table = hisi_sensor_module_dt_match
			},
};
...
int camera_init(void)
{
	...
	ret = platform_driver_register(&k3_sensor_module_probe_driver);
	if (ret != 0) {
		print_error("fail to register k3_sensor_module_probe_driver platform driver !!");
	}
...
void camera_exit(void)
{
	...
	platform_driver_unregister(&k3_sensor_module_probe_driver);
