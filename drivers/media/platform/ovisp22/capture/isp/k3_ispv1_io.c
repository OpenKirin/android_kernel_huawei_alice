 ...
 **************************************************************************
 */
int ispv1_idconfig(camera_pull_state pull_type)
{
    int ret = 0;
    print_info("%s ", __func__);

    if (IS_ERR_OR_NULL(ispv1_io.module_pinctrl) || IS_ERR_OR_NULL(ispv1_io.module_pinctrl_def_UP) || IS_ERR_OR_NULL(ispv1_io.module_pinctrl_idle_DN)){
            print_error("%s: module_pinctrl or module_pinctrl_def_UP or module_pinctrl_idle_DN is NULL\n", __func__);
            return -EINVAL;
    }
    else{
            if (pull_type == PULL_UP) {
               ret = pinctrl_select_state(ispv1_io.module_pinctrl, ispv1_io.module_pinctrl_def_UP); //work mode
               if (ret != 0) {
                     print_error("%s: failed to set iomux id pin to module_pinctrl_def_UP mode.\n", __func__);
               }
          } else {
               ret = pinctrl_select_state(ispv1_io.module_pinctrl, ispv1_io.module_pinctrl_idle_DN);
               if (ret != 0) {
                    print_error("%s: failed to set iomux id pin  to module_pinctrl_idle_DN mode.\n", __func__);
               }
          }
    }
    return ret;
 ...
 **************************************************************************
 */
int ispv1_io_hw_init(struct platform_device *pdev, struct platform_device *module_pdev)
{
	int ret = 0;
    ...
    if (IS_ERR_OR_NULL(module_pdev)){
          print_error("%s : module_pdev is NULL\n", __FUNCTION__);
    }
    else{
        /* module pinctrl enable */
        ispv1_io.module_pinctrl = devm_pinctrl_get(&module_pdev->dev);
        if (IS_ERR_OR_NULL(ispv1_io.module_pinctrl)) {
            print_error("%s :could not get module_pinctrl\n",__FUNCTION__);
        }
        else{
            ispv1_io.module_pinctrl_def_UP = pinctrl_lookup_state(ispv1_io.module_pinctrl,PINCTRL_STATE_DEFAULT);
            if (IS_ERR_OR_NULL(ispv1_io.module_pinctrl_def_UP)) {
                print_error("%s : could not get module_pinctrl_def_UP state\n", __FUNCTION__);
            }
            ispv1_io.module_pinctrl_idle_DN = pinctrl_lookup_state(ispv1_io.module_pinctrl,PINCTRL_STATE_IDLE);
            if (IS_ERR_OR_NULL(ispv1_io.module_pinctrl_idle_DN)) {
                print_error("%s : could not get module_pinctrl_idle_DN state\n", __FUNCTION__);
            }
        }
     }
...
static void ispv1_io_set_default(void)
{
	...
	ispv1_io_ctl.idconfig		= ispv1_idconfig;
