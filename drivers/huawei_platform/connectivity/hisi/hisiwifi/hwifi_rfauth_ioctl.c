...
int32 wl_get_rx_info(struct cfg_struct *cfg, uint16 us_wid)
{
    struct sk_buff                *skb;
...
int32 wl_rx_count_get(struct cfg_struct *cfg)
{
    ...
    ret = wl_get_rx_info(cfg, WID_INT_RESERVE);
    if(SUCC != ret)
    {
...
int32 wl_rx_rssi_get(struct cfg_struct *cfg)
{
    int32 ret;
    ...
    cfg->android_ioctl.rx_rssi_read_flag = -EFAIL;

    ret = wl_get_rx_info(cfg, WID_SHORT_RESERVE);
    if(SUCC != ret)
    {
        HWIFI_WARNING("Failed to get rx rssi!");
        return -EFAIL;
    }

    ret = wait_event_interruptible_timeout(cfg->wait_queue, (-EFAIL != cfg->android_ioctl.rx_rssi_read_flag), 5 * HZ);
    if (0 == ret)
    {
        HWIFI_WARNING("wait for rx rssi info message time out(5s)!");
        return -EFAIL;
    }
    else if (ret < 0)
    {
        HWIFI_WARNING("wait for rx rssi info message error!");
        return -EFAIL;
    }

    HWIFI_DEBUG("report rx rssi info is %d", cfg->android_ioctl.rx_rssi);

    return cfg->android_ioctl.rx_rssi;
}

int32 wl_set_always_tx_param(struct cfg_struct *cfg, uint32 param)
{
    struct sk_buff          *skb;
    struct hwifi_msg_header *msghdr;
    struct int_wid          *rx_count_wid;
    ...
    msg_size = sizeof(struct hwifi_msg_header) + sizeof(struct int_wid) + sizeof(struct int_wid);
    ...
    /* fill msg address */
    rx_count_wid = (struct int_wid *)skb_put(skb, sizeof(struct int_wid));
    hwifi_fill_int_wid(rx_count_wid, WID_MEMORY_ADDRESS, 0x80008808); /* 常发模式需要配置此地址值为:0x22C5,其余情况配置为0x22D5 */

    /* fill msg val */
    rx_count_wid = (struct int_wid *)skb_put(skb, sizeof(struct int_wid));
    hwifi_fill_int_wid(rx_count_wid, WID_MEMORY_ACCESS_32BIT, param);

    ret = hwifi_send_cmd(cfg, skb);
    if (SUCC != ret)
    {
       HWIFI_WARNING("Failed to send 0x80008808 msg!");
       return -EFAIL;
    }
...
int32 hwifi_android_ioctl_cmd(struct hi110x_device* hi110x_dev, struct ifreq *ifr, int32 cmd)
{
    ...
    switch(ioctl_data.cmd)
    {
        ...
        case HWIFI_ANDROID_IOCTL_CMD_RX_COUNT_GET:
            ...
            break;
        case HWIFI_ANDROID_IOCTL_CMD_RX_RSSI_GET:
            ioctl_data.pri_data.rx_rssi = wl_rx_rssi_get(hi110x_dev->cfg);
            HWIFI_INFO("rx rssi get : %d(dBm*10)", ioctl_data.pri_data.rx_rssi);
            if(copy_to_user(ifr->ifr_data, &ioctl_data, sizeof(wifi_android_ioctl_data_struct)))
            {
               HWIFI_WARNING("Failed to copy to user!");
               ret = -EFAIL;
            ...
            break;
        case HWIFI_ANDROID_IOCTL_CMD_ALWAYS_TX_CONFIG:
            HWIFI_INFO("set always tx mode param, param = 0x%X...", ioctl_data.pri_data.always_tx_param);
            ret = wl_set_always_tx_param(hi110x_dev->cfg, ioctl_data.pri_data.always_tx_param);
            break;
        default:
