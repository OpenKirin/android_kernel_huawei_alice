...
int32 hwifi_recv_rssi_info(struct cfg_struct *cfg, uint8 *buf, int32 len)
{
    if ((NULL == cfg)||(NULL == buf))
    {
        HWIFI_WARNING("Invalid NULL params!");
    ...
    cfg->android_ioctl.rx_rssi = buf[0] | (buf[1] << 8);
    cfg->android_ioctl.rx_rssi_read_flag = SUCC;

    HWIFI_DEBUG("rx_rssi_read_flag :%d", cfg->android_ioctl.rx_rssi_read_flag);
    wake_up_interruptible(&cfg->wait_queue);
    ...
    while(msg_len >= sizeof(struct char_wid))
    {
        ...
        switch(wid)
        {
            ...
            case WID_INT_RESERVE:
                ...
                break;
            case WID_SHORT_RESERVE:
                ret = hwifi_recv_rssi_info(cfg, pos + 3, wid_value_len);
                break;
            default:
                break;
        }
