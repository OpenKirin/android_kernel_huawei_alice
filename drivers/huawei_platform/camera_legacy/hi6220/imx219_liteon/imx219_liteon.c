...
#define IMX219_LITEON_EQUIVALENT_FOCUS		27 // 27mm
...
#define IMX219_LITEON_ZSL    (0x00)//(1 << CAMERA_ZSL_OFF)
...
static framesize_s imx219_liteon_framesizes[] = {
       //1600x1200 29.7fps
      {0, 0, 1600, 1200, 3448, 2009, 30, 30, 0x255, 0x1F1, 0x100, VIEW_FULL, RESOLUTION_4_3, false, false, ECGC_TYPE_MAX, {imx219_liteon_framesize_quarter, ARRAY_SIZE(imx219_liteon_framesize_quarter)}, CLK_900M},
       //3264X2448 15.0fps
      {0, 0, 3264, 2448, 3448, 2608, 15, 15, 0x187, 0x146, 0xA8, VIEW_FULL, RESOLUTION_4_3, false, false, ECGC_TYPE_BSHUTTER_LONG, {imx219_liteon_framesize_full, ARRAY_SIZE(imx219_liteon_framesize_full)}, CLK_650M },
};
...
static int imx219_liteon_read_otp_page(unsigned int page, unsigned int group_flag)
{
    ...
    if(group_flag==0){   //for group one otp data
            ...
            {
                imx219_liteon_read_reg(0x3204+i, (unsigned char*)&(otp_tmp_buf[otp_offset++]));
            }
    }
    else{   //for group two otp data
            for(i=0;i<28;i++)   //group 4 only 28 valid data
            {
                imx219_liteon_read_reg(0x3228+i, (unsigned char*)&(otp_tmp_buf[otp_offset++]));
            }
        }
    }
...
/*
 **************************************************************************
 * FunctionName: imx219_liteon_dump_reg_debug;
...
static void imx219_liteon_dump_reg_debug(void)
{
     ...
     reg=0x0100;imx219_liteon_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);//stream on
     reg=0x0018;imx219_liteon_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);// frame counter
     reg=0x016C;imx219_liteon_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);//X output size H
     reg=0x016D;imx219_liteon_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);//X output size L
     reg=0x016E;imx219_liteon_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);//Y output size H
     reg=0x016F;imx219_liteon_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);//Y output size L
     reg=0x0162;imx219_liteon_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);//HTS H
     reg=0x0163;imx219_liteon_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);//HTS L
     reg=0x0160;imx219_liteon_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);//VTS H
     reg=0x0161;imx219_liteon_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);//VTS L
}
...
static int imx219_liteon_check_sensor(void)
{
	...
	if(ret < 0) {
		print_error("imx219_liteon_sensor fail to get gpio value!!!so return -1!\n");
		return -1;
	}

	if(ret != MODULE_LITEON){
		print_info("%s this moudle is not imx219_liteon moudle",__func__);
		return -1;
	}
...
/*
 **************************************************************************
 * FunctionName: imx219_liteon_config_dphy_clk;
...
/*
 **************************************************************************
 * FunctionName: imx219_liteon_get_sensor_reg;
...
static void imx219_liteon_get_sensor_reg(int reg, int *pvalue)
{
	u8 value = 0;
	if (NULL == pvalue)
	{
	...
	imx219_liteon_read_reg(reg, &value);
	*pvalue = value;
...
/*
 **************************************************************************
 * FunctionName: imx219_liteon_set_sensor_reg;
 ...
 **************************************************************************
*/
static void imx219_liteon_set_sensor_reg(int reg, int value)
{
	imx219_liteon_write_reg(reg, value & 0xff, 0x00);
	return;
}
...
static void imx219_liteon_set_default(void)
{
	...
	imx219_liteon_sensor.get_sensor_reg = imx219_liteon_get_sensor_reg;
	imx219_liteon_sensor.set_sensor_reg = imx219_liteon_set_sensor_reg;
