...
const unsigned long data0_camel[COEFFICIENT] = {12638,7636,1945,1810};
const unsigned long data1_camel[COEFFICIENT] = {8117,4438,651,580};
const unsigned long judge_camel[COEFFICIENT] = {1090,1475,1887,3100};
...
#define MAXSET_CASE      (4)
	...
	else if (3 == apds_type)
	{
		for(i=0; i<4; i++)
		{
			data0_temp_coefficient[i] = data0_camel[i];
			data1_temp_coefficient[i] = data1_camel[i];
			judge_temp_coefficient[i] = judge_camel[i];
		}
	}
...
// Theorically it is not necesssary to do so, but I just want to avoid any potential error.  -- Andy 2012.6.6
	...
	if (time_after_eq(jiffies, lastprint_time + PRINT_LUX_PERIOD * HZ ))   //print the lux value per 30s
	{
		...
		als_ps_INFO("[ALS_PS]: the luxval is %d for period,data0=%d,data1=%d\n", als_ps->als_data,als_ps->als_data0_raw,als_ps->als_data1_raw);
	}
	else					//the luxval over last luxsection luxval, also print to kmsg
	{
		...
		if ((luxsection != i) && (abs(lastluxvalue-als_ps->als_data) >= lux_stepbuff))
		{
			...
			als_ps_INFO("[ALS_PS]: the luxval is %d for luxsection skip to section%d.data0=%d,data1=%d\n", als_ps->als_data,luxsection,als_ps->als_data0_raw,als_ps->als_data1_raw);
		}
	}
...
static int rpr521_init_client(struct i2c_client *client)
{
   ...
   if (3==apds_type)
   {
       result=Rpr0521_i2c_write(client, REG_MODECONTROL, (PS_ALS_SET_MODE_CONTROL|0x20),RPR0521_I2C_BYTE);//
       if(result<0)
       {
          goto exit;
       }
   ...
   if (3==apds_type)
   {
       result=Rpr0521_i2c_write(client, REG_PERSISTENCE, (PSGAIN_X4 | 0x2),RPR0521_I2C_BYTE);
	   if(result<0)
       {
          goto exit;
       }
...
#define ROHM_PSALS_PSMAX  (4095)
	...
	als_ps_INFO("[ALS_PS]apds_type is :%d\n", apds_type);
	...
	else if(3 == apds_type)		//CAMEL
	{
		rpr0521_pwave_value  = 110;
		rpr_threshold_value = 90;
		rpr0521_pwindows_value = 230;
	}
	else
	{
