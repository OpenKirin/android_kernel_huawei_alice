...
STATIC void hi6521_config_opt_param(struct hi6521_device_info *di)
{
	...
	else
	{
	    ...
	    if( 0 != customized_config_by_product ){
	        /*Modify flash_bst_vo from 4.5V to 4.8V, need by audio*/
	        hi6521_write_byte(di, 0x07, 0x5f);
	        /*Modify flash_led_vbatdroop from 3.5V to 3.3V, need by camera*/
	        hi6521_write_byte(di, 0x3a, 0x62);
	        /*Modify flash_led_isafe from 200mA to 400mA, need by camera*/
	        hi6521_write_byte(di, 0x07, 0x67);
	    }else{
	        //use default configuration,do nothing
	    }
	}
...
STATIC ssize_t hi6521_show_chargerType(struct device *dev,
                  struct device_attribute *attr,
                  char *buf)
{
    struct hi6521_device_info *di = dev_get_drvdata(dev);
...
STATIC DEVICE_ATTR(chargerType, (S_IWUSR | S_IRUGO),
                hi6521_show_chargerType,
                NULL);
                           ...
                           const struct i2c_device_id *id)
{
    ...
    /*Judge if the product need some special configuration*/
    if (of_property_read_u32(np, "redefine_reg_configuration",
        &customized_config_by_product) < 0) {
        hwlog_info("%s no product config item, use platform config!\n", __func__);
    }else{
        hwlog_info("%s some config item has been configed by product!\n", __func__);
    }
