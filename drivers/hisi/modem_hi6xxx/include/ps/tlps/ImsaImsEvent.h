...
enum IMSA_IMS_INT_ERROR_CODE_ENUM
{
    ...
    IMSA_IMS_INT_ERROR_CODE_CALL_SRVCC_FAIL_BACK_FAIL,后发生srvcc切换失败冲协商没有完成接听发送480拆呼叫，给IMSA 上报错误码 
...
typedef struct {
    ...
    IMSA_IMS_CALL_ADDRESS_TYPE_ENUM_UINT32  enCallAddreeType; /* The 'type' of address. */
    VOS_CHAR                                acNumber[IMSA_IMS_NUMBER_STRING_SZ + 1]; /* The address of the remote party. */
    VOS_UINT8                               ucNeedActiveFlag; /* 0 = not need; 1 = need */
    VOS_UINT8                               aucRsv2[2];
...
typedef struct {
    ...
    VOS_UINT8                                 ucHifiStatus;              /* 0 = not start hifi. 1=  start hifi */
    VOS_UINT8                                 aucRsv[3];
