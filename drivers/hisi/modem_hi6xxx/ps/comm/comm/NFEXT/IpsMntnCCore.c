...
*****************************************************************************/
VOS_VOID IPS_MNTN_CCORE_SetDebug
(
    VOS_UINT32                              ulFlag,
    TCPIP_LOG_LEVEL_E                       enLogLev
)
{
    if (VOS_YES == ulFlag)
    {
        ...
        (VOS_VOID)TCPIP_SetLogLevel(enLogLev);
...
VOS_VOID IPS_MNTN_CCORE_TraceAdvancedCfgReq(VOS_VOID *pMsg)
{
    ...
    if (IPS_MNTN_CCORE_TRACE_CONFIGURABLE_LEN_CHOSEN <= g_stIpsCcoreTraceMsgCfg.stInputTraceCfg.ulChoice)
    {
        IPS_MNTN_CCORE_SetDebug(VOS_YES, TCPIP_LOG_LEVEL_WARNING);
    }
    else
    {
        IPS_MNTN_CCORE_SetDebug(VOS_NO, TCPIP_LOG_LEVEL_WARNING);
    }
