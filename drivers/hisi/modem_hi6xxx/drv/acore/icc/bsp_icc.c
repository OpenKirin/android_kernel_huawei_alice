...
/*portlandC55 VoLTE，为IMS VOIP封装接口以及ICC通道，begin*/
...
{
    ICC_CHAN_ATTR_S attr = {0};
    BSP_S32 ret = 0;

    if (MDRV_ICC_VT_VOIP == u32ChanId)
    {
        ...
        attr.event_cb = pChanAttr->event_cb;
        attr.read_cb = pChanAttr->read_cb;
        attr.write_cb = pChanAttr->write_cb;

        ret = BSP_ICC_Open(ICC_CHN_VT_VOIP, &attr);
		if (BSP_OK != ret)
        {
            printk(KERN_ERR "mdrv_icc_open failed! ret=%d.\n",ret);
        }

        return ret;
    }

    printk(KERN_ERR "mdrv_icc_open channel[%d] invalid.\n",u32ChanId);
    return BSP_ERROR;
}
/************************************************************************
 * 函 数 名  : mdrv_icc_read
...
int mdrv_icc_read(unsigned int u32ChanId, unsigned char* pData, int s32Size)
{
    ...
    if ((MDRV_ICC_VT_VOIP == u32ChanId) || (ICC_CHN_VT_VOIP == u32ChanId))
    {
        ret = BSP_ICC_Read(ICC_CHN_VT_VOIP, pData, s32Size);
		if (BSP_OK != ret)
        {
            printk(KERN_ERR "mdrv_icc_read failed! channel[%d],ret=%d.\n",u32ChanId,ret);
        }

        return ret;
    }

    printk(KERN_ERR "mdrv_icc_read channel[%d] invalid.\n",u32ChanId);
    return BSP_ERROR;
}
...
int mdrv_icc_write(unsigned int u32ChanId, unsigned char* pData, int s32Size)
{
    ...
    if (MDRV_ICC_VT_VOIP == u32ChanId)
    {
        ret = BSP_ICC_Write(ICC_CHN_VT_VOIP, pData, s32Size);
        if (BSP_OK != ret)
        {
            printk(KERN_ERR "mdrv_icc_write failed! ret=%d.\n",ret);
        }
